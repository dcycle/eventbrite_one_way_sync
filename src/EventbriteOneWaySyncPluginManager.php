...
/**
 * A plugin manager for EventbriteOneWaySyncPlugin plugins.
 *
 * The EventbriteOneWaySyncPluginManager class extends the DefaultPluginManager
 * to provide a way to manage EventbriteOneWaySyncPlugin plugins. A plugin
 * manager defines a new plugin type and how instances of any plugin of that
 * type will be discovered, instantiated and more.
 *
 * Using the DefaultPluginManager as a starting point sets up our
 * EventbriteOneWaySyncPlugin plugin type to use annotated discovery.
 *
 * The plugin manager is also declared as a service in
 * eventbrite_one_way_sync.services.yml so that it can be easily accessed and
 * used anytime we need to work with EventbriteOneWaySyncPlugin plugins.
 *
...
class EventbriteOneWaySyncPluginManager extends DefaultPluginManager {
// @codingStandardsIgnoreEnd
  ...
  // @codingStandardsIgnoreEnd
    ...
    // 'src/Plugin/EventbriteOneWaySyncPlugin' subdirectory of any enabled
    ...
    // such as Drupal\{module_name}\Plugin\EventbriteOneWaySyncPlugin\MyPlugin.
    $subdir = 'Plugin/EventbriteOneWaySyncPlugin';
    ...
    $plugin_interface = EventbriteOneWaySyncPluginInterface::class;

    // The name of the annotation class that contains the plugin definition.
    $plugin_definition_annotation_name = EventbriteOneWaySyncPluginAnnotation::class;
    ...
    // backend. For our EventbriteOneWaySyncPlugin plugin type, we've specified
    ...
    // cache_default table using the eventbrite_one_way_sync_plugin key. All
    // that is implementation details however, all we care about it that
    // caching for our plugin definition is taken care of by this call.
    $this->setCacheBackend($cache_backend, 'eventbrite_one_way_sync_plugin');
  }
